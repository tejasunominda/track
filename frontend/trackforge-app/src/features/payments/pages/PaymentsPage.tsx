...
export function PaymentsPage() {
  ...
  const [items, setItems] = useState([
    { id: "p-1", description: "Annual subscription", amount: 1200, status: "Paid" },
    { id: "p-2", description: "Add-on seats", amount: 300, status: "Paid" },
  ]);
  ...
  const add = (e: React.FormEvent) => {
    ...
    setItems((prev) => [...prev, { id: `p-${Date.now()}`, description: desc, amount: parseInt(amount), status: "Paid" }]);
    setDesc("");
    setAmount("");
    setShow(false);
      ...
      <div className="divide-y rounded-xl border border-slate-200 bg-white shadow-sm">
        {items.map((i) => (
          <div key={i.id} className="flex items-center justify-between p-4 transition-all duration-150 hover:bg-slate-50">
            ...
            <span className={`flex items-center gap-1 rounded px-2 py-0.5 text-[10px] font-bold uppercase ${i.status === "Paid" ? "bg-green-100 text-green-700" : "bg-yellow-100 text-yellow-700"}`}>
              <CheckCircle className="h-3 w-3" /> {i.status}
            </span>
          </div>
        ))}
      </div>
