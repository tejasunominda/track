...
export function DocumentsPage() {
  ...
  const [items, setItems] = useState([
    { id: "d-1", title: "Doc A", status: "Published" },
    { id: "d-2", title: "Doc B", status: "Draft" },
  ]);
  ...
  const add = (e: React.FormEvent) => {
    ...
    setItems((prev) => [...prev, { id: `d-${Date.now()}`, title, status: "Draft" }]);
    setTitle("");
    setShow(false);
    ...
    <div className="h-full bg-slate-50 p-6 animate-fadeIn">
      ...
      {show && (
        <form onSubmit={add} className="mb-6 flex gap-2 rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
          <input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Document title" className="flex-1 rounded-lg border border-slate-200 px-3 py-2 text-sm outline-none focus:border-blue-500" />
