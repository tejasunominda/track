...
export function OnboardingPage() {
  ...
  const [items, setItems] = useState([
    { id: "on-1", name: "Alice", role: "Engineer", status: "In progress" },
    { id: "on-2", name: "Bob", role: "Designer", status: "Pending" },
  ]);
  ...
  const add = (e: React.FormEvent) => {
    ...
    setItems((prev) => [...prev, { id: `on-${Date.now()}`, name, role, status: "Pending" }]);
    setName("");
    setRole("");
    setShow(false);
    notify("Onboarding started");
  };
        ...
        {items.map((i) => (
          <div key={i.id} className="flex items-center justify-between p-4 transition-all duration-150 hover:bg-slate-50">
            ...
            <div className="flex items-center gap-2">
              <span className={`rounded px-2 py-0.5 text-[10px] font-bold uppercase ${i.status === "Completed" ? "bg-green-100 text-green-700" : i.status === "In progress" ? "bg-blue-100 text-blue-700" : "bg-slate-100 text-slate-600"}`}>{i.status}</span>
              {i.status !== "Completed" && (
                <button onClick={() => complete(i.id)} className="flex items-center gap-1 rounded-lg bg-blue-600 px-2 py-1 text-xs font-semibold text-white hover:bg-blue-700">
